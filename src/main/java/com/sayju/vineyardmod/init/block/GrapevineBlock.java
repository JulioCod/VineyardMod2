...
public class GrapevineBlock extends BushBlock implements IGrowable {
    ...
    public GrapevineBlock() {
        ...
        this.setDefaultState(this.stateContainer.getBaseState().with(AGE, valueOf(0)));
    }
    ...
    public void tick(BlockState state, World worldIn, BlockPos pos, Random random) {
        ...
        int i = state.get(AGE);
        if (i < 3 && worldIn.getLightSubtracted(pos.up(), 0) >= 9 && net.minecraftforge
                .common.ForgeHooks.onCropsGrowPre(worldIn, pos, state, random.nextInt(5) == 0)) {
            worldIn.setBlockState(pos, state.with(AGE, valueOf(i + 1)), 2);
    ...
    public boolean onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        ...
        if (!flag && player.getHeldItem(handIn).getItem() == Items.BONE_MEAL) {
            ...
            worldIn.setBlockState(pos, state.with(AGE, valueOf(1)), 2);
            return true;
        } else {
    ...
    public void grow(World worldIn, Random rand, BlockPos pos, BlockState state) {
        ...
        worldIn.setBlockState(pos, state.with(AGE, valueOf(i)), 2);
    }
}
